import {Link, useParams} from 'react-router-dom';
import {useEffect, useState} from 'react';
import {Customer} from "../../types/Customer.ts";

function CustomerDetailPage() {
  const {id} = useParams();
  const [customer, setCustomer] = useState<Customer>();

  const currencySymbols = {
	'dollars': '$',
	'euros': '€',
  };

  useEffect(() => {
	// Fetch customer data based on the id
	fetch(`${import.meta.env.VITE_API_BASE_URL}/customers/${id}/purchases`)
		.then(response => response.json())
		.then(data => setCustomer(data));
  }, [id]);

  if (!customer) {
	return <div>
	  <div role="status">
		<svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
			 viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
		  <path
			  d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
			  fill="currentColor"/>
		  <path
			  d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
			  fill="currentFill"/>
		</svg>
		<span className="sr-only">Loading...</span>
	  </div>
	</div>;
  }

  return (
	  <div className="container mx-auto px-4 sm:px-8">
		<div className="py-8">
		  <div className="flex flex-row mb-1 sm:mb-0 justify-between w-full">
			<h2 className="text-2xl leading-tight">Customer Detail</h2>
			<Link to="/customers"
				  className="transition text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
			  Retour à la liste des clients
			</Link>
		  </div>
		  <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
			<div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
			  <table className="min-w-full leading-normal">
				<thead>
				<tr>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Last Name
				  </th>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Purchase Identifier
				  </th>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Product ID
				  </th>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Quantity
				  </th>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Price
				  </th>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Currency
				  </th>
				  <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
					Date
				  </th>
				</tr>
				</thead>
				<tbody>
				{customer.purchases.map((purchase) => (
					<tr key={purchase.purchase_identifier}>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{customer.lastname}
					  </td>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{purchase.purchase_identifier}
					  </td>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{purchase.product_id}
					  </td>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{purchase.quantity}
					  </td>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{purchase.price}
					  </td>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{purchase.currency}
					  </td>
					  <td className="px-5 py-5 border-b border-gray-200 bg-white dark:bg-black text-sm">
						{new Date(purchase.date).toLocaleDateString()}
					  </td>
					</tr>
				))}
				</tbody>
			  </table>
			  <div className="px-5 py-5 border-t border-gray-200 bg-white dark:bg-black text-sm">
				{Object.entries(customer.purchases.reduce((acc, purchase) => {
				  acc[purchase.currency] = (acc[purchase.currency] || 0) + purchase.price * purchase.quantity;
				  return acc;
				}, {})).map(([currency, total]) => (
					<p key={currency} className="text-right font-semibold">
					  Total en {currency}: {total} {currencySymbols[currency]}
					</p>
				))}
			  </div>
			</div>
		  </div>
		</div>
	  </div>
  );
}

export default CustomerDetailPage;